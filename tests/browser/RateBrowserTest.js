...
test('core-rate count', async t => {
  const rateGroup = await Selector(() => {
    return document.querySelectorAll('core-rate')
  })
  await t
    .expect(rateGroup.count).eql(7)
})
...
test('show-score attribute', async t => {
  ...
  await t
    .click(star)
    .expect(outputTxt.textContent).contains(' points')
})
...
test('text-color attribute', async t => {
  ...
  await t
    .expect(outputTxt.getAttribute('style')).contains('text-color')
})
...
test('texts attribute', async t => {
  ...
  await t
    ...
    .expect(outputTxt.textContent).eql('great')
})
...
test('score-template attribute', async t => {
  ...
  await t
    .click(star)
    .expect(outputTxt.innerText).contains(' points')
})
